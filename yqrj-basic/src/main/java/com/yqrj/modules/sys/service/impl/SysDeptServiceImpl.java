...
import com.yqrj.modules.sys.dao.SysDeptDao;
...
@Service
public class SysDeptServiceImpl extends BaseServiceImpl<SysDeptDao, SysDeptEntity>
    ...
    public List<SysDeptDTO> list(Map<String, Object> params) {
        ...
        List<SysDeptEntity> entityList = baseDao
            .selectList(new EntityPluWrapper<SysDeptEntity>(params));

        return ConvertUtils.sourceToTarget(entityList, SysDeptDTO.class);
    }

    @Override
    public List<SysDeptDTO> listSql(Map<String, Object> params) {
        //普通管理员，只能查询所属部门及子部门的数据
        UserDetail user = SecurityUser.getUser();
        if (user.getSuperAdmin() != null && user.getSuperAdmin() == SuperAdminEnum.NO.value()) {
    ...
    public void update(SysDeptDTO dto) {
        ...
        if (entity.getId().equals(entity.getPid())) {
            throw new YqrjException(ErrorCode.SUPERIOR_DEPT_ERROR);
        }
    ...
    public void delete(Long id) {
        ...
        if (subList.size() > 1) {
            throw new YqrjException(ErrorCode.DEPT_SUB_DELETE_ERROR);
        }

        //判断部门下面是否有用户
        int count = sysUserService.getCountByDeptId(id);
        if (count > 0) {
            throw new YqrjException(ErrorCode.DEPT_USER_DELETE_ERROR);
        }
